...
var _RedisCluster_instances, _a, _RedisCluster_extractFirstKey, _RedisCluster_options, _RedisCluster_slots, _RedisCluster_Multi, _RedisCluster_handleCommandError;
...
class RedisCluster extends events_1.EventEmitter {
    ...
    async commandsExecutor(command, args) {
        ...
        const reply = command.transformReply(await this.sendCommand(__classPrivateFieldGet(RedisCluster, _a, "m", _RedisCluster_extractFirstKey).call(RedisCluster, command, args, redisArgs), command.IS_READ_ONLY, redisArgs, options, command.BUFFER_MODE), redisArgs.preserve);
        return reply;
    }
    ...
    multi(routing) {
        return new (__classPrivateFieldGet(this, _RedisCluster_Multi, "f"))(async (commands, chainId) => {
            ...
            return Promise.all(commands.map(({ args }) => {
                return client.sendCommand(args, client_1.default.commandOptions({
                    chainId
                }));
            }));
...
(0, commander_1.extendWithDefaultCommands)(RedisCluster, RedisCluster.prototype.commandsExecutor);
